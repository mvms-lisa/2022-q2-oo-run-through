-- spotx share calculations for q2 2022 - may not even need steps 1-3? Need to re-evaluate.
    -- in order to calculate spotx share, we need to  
        ...
        group by year_month_day
         ...
         group by year_month_day, month, year, quarter          
           ...
           select * from monthly_gross_revenue where year = 2022 and quarter = 'q2'
           
           select * from spotx where year = 2022 and quarter =  'q2'
        ...
        from(
          ...
          where s.year = 2022 and s.quarter = 'q2' and m.name = 'SpotX Gross Revenue'
        ) q
        where s.id = q.id
        ...
        from(
          ...
          where pay_partner = 'spotx' and s.year = 2022 and s.quarter = 'q2'
        ) q
        where s.id = q.id 
        ...
        -- 4a. NEW ADDITION (8/30/22) - We need to calculate the net_revenue, then use that to calculate the share_exp %, and then use that percentage to calculate the final_net_revenue.
              ...
              call spotx_update_net_revenue('q2', 2022::DOUBLE);
            ...
            -- 4b. Insert the sum of net_revenue by month:
              ...
              select * from monthly_gross_revenue
            -- 4c. Need to calculate the exp_share, which takes the net_revenue per record divided by the sum of net_revenue per month.
                -- Before running this step, the sum of the net_revenue should be inserted into the monthly_gross_revenue table.
                    ...
                    from (
                        ...
                        join monthly_gross_revenue m on (m.year_month_day = s.year_month_day)
                        where s.year = 2022 and s.quarter = 'q2' and m.name = 'SpotX Net Revenue'
                        and m.partner = 'spotx'
                    )q
                    where s.id = q.sid
            ...
            -- 4d. Below will update the final_net_revenue values, which will later be inserted into the monthly_revenue table.
                ...
                -- Updates the final_net_revenue:
                    ...
                    from ( 
                      ...
                      join expenses e 
                      where s.year = 2022 and s.quarter = 'q2' and e.pay_partner = 'spotx' and e.description = 'SpotX Seat Fee' and e.year = 2022 and e.quarter = 'q2'
                    ) q
                    where q.qid = s.id
                    
select * from expenses where year = 2022 and quarter = 'q2' and description = 'SpotX Seat Fee'

insert into expenses(amount, year, quarter, pay_partner, filename, title, description, label, viewership_type)
values(-2500, 2022, 'q2', 'spotx', 'manual', 'SpotX', 'SpotX Seat Fee', 'Expense', 'VOD')